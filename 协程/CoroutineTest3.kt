...
*
*   从[代码角度]上理解同步和异步：
*   协程 通过 将异步程序同步化的方式 来解决多线程中多层嵌套的callback。
*   具体如何实现 异步程序同步化 暂时不需要深入了解。
*   协程在使用中可以摆脱比较跳脱的回调的代码写法，在调用者的逻辑上看，整个程序、逻辑是同步的。
*   代码上：所谓同步可以大致的理解为代码的顺序执行，同步的相对面就是异步。而callBack是一种很典型的异步。
*   代码上同步执行的代码很容易理解。而协程是异步的，所以为了的方便代码的便捷性来说，使异步代码同步化是有必要的、
*
...
*   去Ui的更新。如果使用异步的写法（callBack的写法），那么就需要进行多层callBack函数嵌套。
    ...
    强行将可以并发的程序通过[嵌套]的方式来同步化。这种同步化手段缺点很大，使之前可以并发请求的两个网络请求存在顺序性
    其效率降低。整体时间可以为并发的两倍。
